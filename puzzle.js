...
var d_direct = new Array(
  [0],//为了逻辑更加简单，第一个元素我们不用，我们从下标1开始使用
...
var d_posXY = new Array(
  [0],//同样第一个元素我们不用
  [0,0],//第一个表示left,第二个表示top，比如第一块的位置表示left:0px;top:0px
  [150,0],
  [300,0],
  [0,150],
  [150,150],
...
function move(id){
  ...
  //这个for循环是找出小div在大div的位置
  var target_d = 0;
  //保存小div可以去的编号，0表示不能移动
  target_d = whereCanTo(i);
  //用来找出小div可以去的位置，如果返回0，表示不能移动，否则，儿一科移动
  if(target_d !=0){
    d[i]=0;
    //把当前的大div编号设置为0，因为当前小div已经移走了，
    //所以当前大div就没有装小div
    d[target_d]=id;
  ...
  for(var k=1;k<9;k++){
    if(d[k] != k){
      finish_flag = false;
      break;
      //如果大div保存的编号和它本身的编号不同，则表示还不是全部按照顺序拍的。
  ...
  //如果为true,则表示游戏完成，如果当前没有暂停，则调用暂停模式，
  //并且弹出提示框,完成游戏。
  //start()这个函数是开始，暂停一起的函数，如果暂停，调用会开始，如果开始，则调用会暂停。
}
...
//随机方块打乱函数，我们的思路是从第九快开始，随机生成一个数，然后他们两块对调一下
function random_d(){
  for(var i=9;i>1;--i){
    var to = parseInt(Math.random()*(i-1)+1);//产生随机数，范围1到i，不能超出范围，因为没这个id的div
    if(d[i]!=0){
